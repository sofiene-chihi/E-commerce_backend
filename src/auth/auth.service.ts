...
export class AuthService {
  ...
  async register(registerUserInput: RegisterUserInput): Promise<User> {
    const foundOwner = await this.usersService.findOne(registerUserInput.email);
    if (foundOwner) {
  ...
  async login(loginInput: LoginInput) {
    const user: User = await this.usersService.findOne(loginInput.email);
    if (!user) {
    ...
    const jwt = this.jwtService.signAsync({ sub: user.id, role: user.roleId });
    return {
      user: user,
